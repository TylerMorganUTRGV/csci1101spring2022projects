...
num_of_incorrect_guessses = 0
guessed_letters = []

# Playing the game inside the while loop
while num_of_incorrect_guessses < TOTAL_NUM_OF_INCORRECT_GUESSES_ALLOWED and False in guessed_answer:
  print("--------------------------------------------")
  ...
  print(f"Number of incorrect guesses remaining: {TOTAL_NUM_OF_INCORRECT_GUESSES_ALLOWED - num_of_incorrect_guessses}")
  ...
  if letter not in guessed_letters and len(letter) == 1 and re.search("^[A-Z]$", letter):
    ...
    if letter in answer:
      ...
      num_of_incorrect_guessses += 1

# Post-game summary.
print()

if num_of_incorrect_guessses < TOTAL_NUM_OF_INCORRECT_GUESSES_ALLOWED:
